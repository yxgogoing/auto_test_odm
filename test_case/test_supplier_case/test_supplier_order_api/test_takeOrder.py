...
@parameterized([
    # 正向用例：失败
    param(dict(purchaseOrderNoList=10000), msg="验证订单号不符合规则时报错", code=0),
    param(dict(purchaseOrderNoList=None), msg="验证订单号=None时报错", code=0),
    param(dict(purchaseOrderNoList="abc"), msg="验证该接口订单号=\"abc\"时报错", code=0),
    # 正向用例：成功
